# QFaaS Note for faas-netes

faas-netes for QFaaS project

Version: based on OpenFaas [faas-netes](https://github.com/openfaas/faas-netes) commit `d05bda9`

## Build Instruction

```bash
docker build -t qfaas/faas-netes .
```

## Major Modifications
- Add UDP port support to function Service Spec
    - Please check [pkg/handlers/deploy.go](pkg/handlers/deploy.go) for more details
    - This modification dose not affect the TCP port, should be compatible with the original OpenFaaS
 - We also add a function pod node selector
    - Functions will be deployed on Kubernetes node with label `faastype=fn`
